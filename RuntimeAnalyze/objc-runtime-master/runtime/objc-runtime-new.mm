...
// 获取到一个重映射的哈希表(在程序运行过程中，并不用这个哈希表)
...
// 返回存活的cls类指针，它可能指向一个重新创建的类结构体指针，如果由于弱链接忽略了cls，则返回nil。
...
static NXMapTable *protocols(void)
{
    ...
    // 下面是将protocol_map传进去，这是protocol单独的表
...
static Protocol *getProtocol(const char *name)
{
    ...
    // Try name as-is.
    // 从table->buckets中取出Protocol，这个哈希表是属于Protocol的
