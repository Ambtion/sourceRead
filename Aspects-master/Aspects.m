...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    // 自旋锁OSSpinLockLock 比递归锁@synchronized 性能高
    // 线程安全
    aspect_performLocked(^{
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) {
            
            // 创建AspectsContainer容器；绑定到当前类上;key = aspects_ + NSStringFromSelector(selector)
            AspectsContainer *aspectContainer = aspect_getContainerForObject(self, selector);
            
            // 创建hook切面信息类identifier
            identifier = [AspectIdentifier identifierWithSelector:selector object:self options:options block:block error:error];
            if (identifier) {
                [aspectContainer addAspect:identifier withOptions:options];
                
                // Modify the class to allow message interception.
                // hook 方法转发到_objc_msgForward，触发invocation
...
static NSMethodSignature *aspect_blockMethodSignature(id block, NSError **error) {
    ...
    if (layout->flags & AspectBlockFlagsHasCopyDisposeHelpers) {
        // 如果实现了copy dispose函数，指针往下走2 * sizeof(void *)
        // 直至获取signature 的地址
        desc += 2 * sizeof(void *);
    }
    if (!desc) {
...
static BOOL aspect_isCompatibleBlockSignature(NSMethodSignature *blockSignature, id object, SEL selector, NSError **error) {
    ...
    if (blockSignature.numberOfArguments > methodSignature.numberOfArguments) {
        ...
        if (blockSignature.numberOfArguments > 1) {
            ...
            if (blockType[0] != '@') {
                // 比对第二个参数是否是 id<AspectInfo> 对于encode(@)
                // 第一个参数此处为block 类型 ； "@?"
                signaturesMatch = NO;
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    ...
    // 判断hook方法imp是否是_objc_msgForward or _objc_msgForward_stret
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {
        
        // Make a method alias for the existing method implementation, it not already copied.
        
        // 使用alias seletor 保留原来方法
        const char *typeEncoding = method_getTypeEncoding(targetMethod);
        SEL aliasSelector = aspect_aliasForSelector(selector);
        if (![klass instancesRespondToSelector:aliasSelector]) {
            // aliasSelector绑定原始实现，IMP为原targetMethodIMP
            __unused BOOL addedAlias = class_addMethod(klass, aliasSelector, method_getImplementation(targetMethod), typeEncoding);
            NSCAssert(addedAlias, @"Original implementation for %@ is already copied to %@ on %@", NSStringFromSelector(selector), NSStringFromSelector(aliasSelector), klass);
        }
        
        // We use forwardInvocation to hook in.
        // 替换原有方法imp 到 _objc_msgForward 触发 forwardInvocation
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    ...
    if ([className hasSuffix:AspectsSubclassSuffix]) {
        ...
        /*
         * 对中间类调用 aspect_swizzleClassInPlace
         */
    ...
    if (subclass == nil) {
        ...
        aspect_hookedGetClass(subclass, statedClass); // subclass的isa指向了statedClass
        aspect_hookedGetClass(object_getClass(subclass), statedClass); //subclass的元类的isa，也指向了statedClass。
...
static void __ASPECTS_ARE_BEING_CALLED__(__unsafe_unretained NSObject *self, SEL selector, NSInvocation *invocation) {
    ...
    // prepare aliasSelector 为原始调用；替换invocation.selector 为原始调用
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
     ...
     * 所有继承UIViewController 的类都会调用到hook；使用AspectTracker控制 继承链唯一hook；防止异常case
     * 实例对象(class_isMetaClass(object_getClass(self)) = NO);容器和地址绑定，保证唯一
     */
...
+ (instancetype)identifierWithSelector:(SEL)selector object:(id)object options:(AspectOptions)options block:(id)block error:(NSError **)error {
    ...
    NSCParameterAssert(block); // 检查参数
    NSCParameterAssert(selector);
    
    /*
     * block中获取NSMethodSignature
     */
    ...
    /*
     * blockSignature 全部都赋值给AspectIdentifier
     */
